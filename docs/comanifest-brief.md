...
- **Hold-count integrity for guests (deliberately deferred — 2026-06)** — A guest who clears cookies, uses incognito, or switches browser/device gets a **new Supabase anonymous `user_id`** and can hold the same manifestation again, inflating `join_count`. This is inherent to anonymous auth, **not a bug**. Within one persistent guest session, repeat-holding is already blocked by `unique (user_id, manifestation_id)` on `manifestation_joins` (a second insert returns `23505` → “You're already holding this manifestation”), and withdraw → re-hold nets to zero. There is **no** IP/device/fingerprint dedup, and CAPTCHA only gates *new-session creation*, not each hold. **Decision: leave as-is for now** — the count is a soft signal of collective intention, not a vote that decides anything (aligns with “no promises or guarantees”). Mitigation options if it ever matters: (1) accept it and surface a “signed-in holders” sub-count alongside the raw total; (2) **require email sign-in to hold** (browse/manifest can stay open to guests) — most effective and reuses existing magic-link auth; (3) coarse IP/device dedup or rate-limiting (leaky via shared IPs/VPNs; needs a privacy disclosure in guidelines); (4) CAPTCHA on every hold (friction, not prevention). Revisit before any wider launch or if counts start being gamed.
